...
# bot init
bot = Bot(token='5384149253:AAG9-4lFYu0EJKpYuUGxGAXOQ3I4inETLJ4')
dp = Dispatcher(bot)
...
@dp.message_handler(text=['Казино все'])
...
async def report(message):
    ...
    await bot.send_message(5592294018, f"""
...
async def start_cmd(message):
    ...
    if user_status == 'Helper_Admin':
       await bot.send_message(reply_user_id, f"✄ | <a href='tg://user?id={user_id}'>{reply_user_name}</a>, ваш аккаунт был разблокирован по ID", parse_mode='html')
       await bot.send_message(5592294018, f"⛔️ |Администратор: <a href='tg://user?id={user_id}'>{user_name}</a>\n⚙️ |Действие: Разбан аккаунта по ID\n💈 | Время: навсегда\n👨 |Игроку: <a href='tg://user?id={user_id}'>{reply_user_name}</a>", parse_mode='html')
...
###########################################АДМИН КОМАНДЫ###########################################
    ...
    if message.text.lower() == '+овнер':
       ...
       if user_status == 'Owner':
          await message.reply(f"⛔️ | Разработчик: <a href='tg://user?id={user_id}'>{user_name}</a> выдал права администратора уровня \"OWNER\" игроку <a href='tg://user?id={reply_user_id}'>{reply_user_name}</a>", parse_mode='html')
...
############жиза##########
    if message.text.startswith("+коин"):
       ...
       donate_coins = round(donate_coins[0])

       if int(donate_coins + perevod) >= 99999999999999999999999999999999999999999999999999999999999999999:
          await bot.send_message(message.chat.id, f"🆘 | <a href='tg://user?id={user_id}'>{user_name}</a>,  Достигнул лимит баланса! 9 гвинт!", parse_mode='html')
          return

       if user_status[0] == 'Owner':
          await message.reply(f"⛔️ | Разработчик: <a href='tg://user?id={user_id}'>{user_name}</a> \n⚙️ | Действие: Выдача коинов\n💈 | Количество: {perevod2}$\n👨 | Игроку: <a href='tg://user?id={reply_user_id}'>{reply_user_name}</a>", parse_mode='html')
          cursor.execute(f'UPDATE users SET donate_coins = {donate_coins + perevod} WHERE user_id = "{reply_user_id}"')
          connect.commit() 
          return
       else:
          await message.reply(f"<a href='tg://user?id={user_id}'>{user_name}</a> ,Вы не являетесь администратором бота 👮. Для получение прав администратора обратитесь владельцу ⚠️", parse_mode='html')
...
<a href="https://t.me/bbbqmq_govnocoder">Разработчик</a>
<a href="https://t.me/diamondgamechat">Беседа</a>
          ...
          if balance >= summ:
             if summ > 0:
                ...
                if int(rx) in range(10201, 10210):
                   ...
                   i2 = int(i)
                   i3 = '{:,}'.format(i2)
                   await bot.send_message(5592294018, f"🤵‍♂ | Игрок: <a href='tg://user?id={user_id}'>{user_name}</a>\n🤵‍♀︄ | Игра: Казино\n🎟 | Ставка: {summ2}$\n🧾 | Выигрыш: {i3}$ [10X]", parse_mode='html')
                ...
                if int(rx) in range(10211, 10215):
                   ...
                   i2 = int(i)
                   i3 = '{:,}'.format(i2)
                   await bot.send_message(5592294018, f"🤵‍♂︄ | Игрок: <a href='tg://user?id={user_id}'>{user_name}</a>\n🤵‍♀︄ | Игра: Казино\n🎟 | Ставка: {summ2}$\n🧾 | Выигрыш: {i3}$ [41X]", parse_mode='html')
...
ℹ️ | Чтобы купить Коины, обратитесь к владельцу бота @bbbqmq_govnocoder
...
#################################################### !канал ################################
    if message.text.lower() in ['канал', "!канал", "channel"]:
       ...
       channel_push = InlineKeyboardButton(text='КАНАЛ 🔈', url='https://t.me/modulesforuserbot')
...
async def craft_resurs3(callback: types.CallbackQuery):
   ...
   balance = cursor.execute("SELECT balance from users where user_id = ?",(callback.from_user.id,)).fetchone()
   balance = int(balance[0])
   user_channel_status = await bot.get_chat_member(chat_id="@modulesforuserbot", user_id=callback.from_user.id)
...
Владелец: @bbbqmq_govnocoder 💻
Наша беседа: @diamondgamechat 💬
    ''', reply_markup=help2, parse_mode='html')
